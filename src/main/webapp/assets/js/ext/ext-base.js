					...
					if(C){
						...
						createSequence:function(B,A){
							...
							return function(){
								...
								B.apply(A||this||window,arguments);return D
								}
							},
								...
								if(typeof B!="function"){
									...
									return function(){
										...
										if(B.apply(A||this||window,arguments)===false){
											return }return C.apply(this||window,arguments)
											}
									}
							});
	Ext.applyIf(String,{
...
(function(){
										...
										getRegion:function(E){
											...
											getY:function(E){
												return this.getXY(E)[1]
													...
													return this.getXY(E)[0]},getXY:function(G){
														var F,K,M,N,J=(document.body||document.documentElement);
														G=Ext.getDom(G);
														if(G==J){return[0,0]}if(G.getBoundingClientRect){M=G.getBoundingClientRect();
														N=C(document).getScroll();
														return[M.left+N.left,M.top+N.top]}var O=0,L=0;
														F=G;
														var E=C(G).getStyle("position")=="absolute";
														while(F){O+=F.offsetLeft;L+=F.offsetTop;
														...
														if(Ext.isGecko&&!E){
															...
															L+=parseInt(I.getStyle("borderTopWidth"),10)||0}F=G.parentNode;
															while(F&&F!=J){
	...
	Ext.lib.Event=function(){
		...
		return{
			...
			WFN:3,
			OBJ:3,
			ADJ_SCOPE:4,_interval:null,
			startInterval:function(){
				if(!this._interval){
					var L=this;var M=function(){
						L._tryPreloadAttach()
						};
						...
						if(!Q||!P){
							...
							var O=function(R){
								return typeof Ext!="undefined"?P(Ext.lib.Event.getEvent(R)):false
										...
										return true},removeListener:function(S,O,R){
											...
											getTarget:function(N,M){
												N=N.browserEvent||N;var L=N.target||N.srcElement;
						...
						if(F){
							...
							if(F.xmlData){
								...
								for(var L=0;L<F.elements.length;L++){
									...
									break;
									case"radio":case"checkbox":if(G.checked){
										K+=encodeURIComponent(E)+"="+encodeURIComponent(H)+"&"
										}
									break;
									case"file":case undefined:case"reset":case"button":break;case"submit":if(M==false){
										K+=encodeURIComponent(E)+"="+encodeURIComponent(H)+"&";
										M=true
										}
									break;
									default:K+=encodeURIComponent(E)+"="+encodeURIComponent(H)+"&"
									;break
									}
									}
									}
								...
								useDefaultXhrHeader:true,defaultXhrHeader:"XMLHttpRequest",
														...
														asyncRequest:function(I,F,H,E){
															...
															if(!G){
																...
																if(this.hasDefaultHeaders||this.hasHeaders){
																	this.setHeader(G)}this.handleReadyState(G,H);
																	G.conn.send(E||null);
																	return G
																	}
															},
																...
																this.poll[F.tId]=window.setInterval(function(){
																	if(F.conn&&F.conn.readyState==4){
																		...
																		if(G&&G.timeout){
																			...
																			delete E.timeout[F.tId]}E.handleTransactionResponse(F,G)
																			}
																	},
																	this.pollInterval)
																		...
																		if(G>=200&&G<300){
																			...
																			break;
																			default:F=this.createResponseObject(I,J.argument);
																			if(J.failure){
																				if(!J.scope){J.failure(F)
																					}
																				else{
																					J.failure.apply(J.scope,[F])
																					}
			...
			Ext.lib.Region=function(G,H,E,F){
				...
				Ext.lib.Region.getRegion=function(H){
					...
					Ext.lib.Point=function(E,F){
									...
									motion:function(H,F,I,J,E,G){
										return this.run(H,F,I,J,E,G,Ext.lib.Motion)
											...
											return this.run(H,F,I,J,E,G,Ext.lib.ColorAnim)
												...
												G=G||Ext.lib.AnimBase;if(typeof L=="string"){
													L=Ext.lib.Easing[L]
													}
												var J=new G(I,F,K,L);
												J.animateX(function(){
						...
						function C(E){
							if(!B){
								B=new Ext.Element.Flyweight()}B.dom=E;return B
								}
						if(Ext.isIE){
							function A(){
